...
for test_index, train_index in tqdm(skf.split(gt,np.ones(len(gt)))):
    H = gt[train_index]
    test = gt[test_index]
    r = get_rwr(A_list, H) # run rwr
    tmp_ph, tmp_hh, tmp_mrr, tmp_t = hot(r, [], A_list, mu_list, test, alpha, num_cluster, lp, K_list = K_list)
    ph.append(tmp_ph)
    hh.append(tmp_hh)
    mrr.append(tmp_mrr)
    t.append(tmp_t)
